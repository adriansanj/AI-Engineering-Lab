...
def generate_ssh_key():
    ...
    auth = (API_KEY, "")
    open_file = "src/lambda/ssh-key/michael-private_key.pem"

    headers = {"Content-Type": "application/json"}
    data = {"name": "michael-public-key"}
...
def get_michael_vm_id():
    """Get the IP address of the Michael VM."""
    intances = list_intances()
    for instance in intances["data"]:
        if instance["name"] == "michael-finetune-instance":
            try:
                return instance["id"]
            except KeyError:
                print("Michael VM IP not found")
    return


def get_michael_vm_ip():
    """Get the IP address of the Michael VM."""
    intances = list_intances()
    for instance in intances["data"]:
        if instance["name"] == "michael-finetune-instance":
            try:
                print(f"Michael VM IP: {instance['ip']}")
            except KeyError:
                print("Michael VM IP not found")
    return
...
def terminate_instances():
    """Terminate the Michael VM."""
    url = f"{BASE_URL}/instance-operations/terminate"
    auth = (API_KEY, "")
    headers = {"Content-Type": "application/json"}

    instance_id = get_michael_vm_id()
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  get-ip         - Get Michael VM IP")
